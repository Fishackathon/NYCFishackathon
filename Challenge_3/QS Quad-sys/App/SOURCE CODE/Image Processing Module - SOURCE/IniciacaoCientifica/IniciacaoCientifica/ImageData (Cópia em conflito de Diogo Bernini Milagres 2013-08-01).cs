...
namespace IniciacaoCientifica
{
    ...
    {
        //dados da imagem
        private double[][,] imageData;//cria um vetor [] de matrizes [,]
        ...
        public double getPixel(int linha, int coluna, int canal)
        {
            return this.imageData[canal][linha, coluna];
        }

        public void setPixel(int linha, int coluna, int canal, double valor)
        {
        ...
        public ImageData(Bitmap bmp)//carrega imagem
        {
            ...
            this.channels = 3;
            this.imageData = new double[3][,];
            for (int i = 0; i < 3; i++)
                this.imageData[i] = new double[bmp.Height, bmp.Width];
            ...
            unsafe
            {
                ...
                for (int i = 0; i < bitmapData1.Height; i++)
                {
                    for (int j = 0; j < bitmapData1.Width; j++)
                    {
                        ...
                        imageData[2][i, j] = (double)imagePointer1[0];//Canal B
                        imageData[1][i, j] = (double)imagePointer1[1];//Canal G
                        imageData[0][i, j] = (double)imagePointer1[2];//Canal R
                        //4 bytes per pixel
                        imagePointer1 += 4;
                    }//end for j
                    //4 bytes per pixel
